...
{
    CLR(tmp);
    rep(i,n)tmp[i]=vertical[scnd][i]-vertical[frst-1][i];
...
int findmaximumsum()
{
    ...
    rep(i,n)
    {
        rep(j,n)vertical[i][j]=vertical[i-1][j]+arr[i][j];
    }
    ///then find horizontal cum sum to find the total cum sum
//    rep(i,n)
//    {
//        rep(j,n)cum[i][j]=cum[i][j-1]+vertical[i][j];
//    }
