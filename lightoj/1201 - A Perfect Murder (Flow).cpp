...
void dfs(int src,int col = 0,int par = -1)
{
//    debug(src)
    color[src] = col;
    repI(i,adj[src].size()) {
...
//    ios_base::sync_with_stdio(0);cin.tie(0);
    ...
    rep(cs,t)
    {
        ...
        SET(color);
        rep(i,n)if(color[i] == -1)dfs(i);
        repI(i,edges.size()) {
            pii x = edges[i];
            if(color[x.ff] == 0)
                mf::addEdge(x.ff,x.ss,1,0);
            else
                mf::addEdge(x.ss,x.ff,1,0);
        }
        rep(i,n)
            if(color[i] == 0)
                mf::addEdge(n+1,i,1,0);
            else
                mf::addEdge(i,n+2,1,0);
        printf("Case %d: %d\n",cs,n-mf::dinitz());
