...
//        WRITE("out.txt");
    #endif // mamun
    fact[0]=1;
    rep(i,mx*2-3)fact[i]=(fact[i-1]*i)%mod;
//    repI(i,mx*2)modinv[i]=modularInverse(fact[i],mod);
    getI(t);
    rep(cs,t)
    {
        getII(n,m);
        ll ans=0;
        ll upore=fact[n+m-1]%mod;
        ans=(upore* modularInverse(fact[n],mod))%mod;
        ans=(ans* modularInverse(fact[m-1],mod))%mod;
        printf("Case %d: %lld\n",cs,ans);
    }
