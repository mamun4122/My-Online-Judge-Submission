...
//    ios_base::sync_with_stdio(0);cin.tie(0);
    ...
    while(~getII(n,m))
    {
        ...
        arr.push_back(pii(n,m));
        sort(ALL(arr));
        k = arr.size();
        /// number of way in n*m grid = c(n-1+m-1, n-1)
        repI(i,k)dp[i] = binomial(arr[i].ff-1+arr[i].ss-1, arr[i].ff-1);
        repI(i,k)
        {
            for(int j = i+1; j < k; j++)
            {
                if(arr[j].ss < arr[i].ss) continue;
                dp[j] -= dp[i] * binomial(arr[j].ff-arr[i].ff+arr[j].ss-arr[i].ss, arr[j].ff-arr[i].ff);
                if(dp[j] < 0) dp[j] = ((dp[j] % MOD) + MOD) % MOD;
            }
        }
//        debug(arr[k-1].ff,arr[k-1].ss)
        printf("%lld\n", dp[k-1]);
