...
//        debug(s,n)
        ...
        CLR(out);
        int tot = n + s.size();
        int src = 0,sink = tot * 2 + 1;
        mf::init(src,sink,sink+1);
        FOR(i,1,tot){
            in[i] = i,out[i] = tot+i;
            mf::addEdge(in[i],out[i],1,0);
        }
        FOR(i,1,n)mf::addEdge(src,in[i],INF,0);
        FOR(i,n+1,tot)mf::addEdge(out[i],sink,INF,0);
        char x;
        rep(i,n)
        {
            rep(j,6){
                cin >> x;
                repI(k,s.size())if(s[k] == x)
                {
                    mf::addEdge(out[i],in[n+k+1],1,0);
//                    debug(i,k+1)
                }
            }
        }
        int ans = mf::dinic();
        if(ans == s.size())puts("YES");
        else puts("NO");
