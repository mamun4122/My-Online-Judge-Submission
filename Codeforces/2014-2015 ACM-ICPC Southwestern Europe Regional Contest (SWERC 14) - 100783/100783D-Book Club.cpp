...
struct dinic_maxflow {
    ...
    void add_edge ( int a, int b, int cap ,int cap2) {
...
//    ios_base::sync_with_stdio(0);cin.tie(0);
    ...
    while(~getII(n,m))
    {
        dinic_maxflow mf(n+n+2, n+n+1, n+n+2);
        //mf::init(n+n+1, n+n+2, n+n+2);
        FOR(i,1,m)
        {
            int x,y;
            getII(x,y);
            mf.add_edge(x+1, n+y+1, 1, 0);
        }
        FOR(i,1,n) mf.add_edge(n+n+1, i, 1, 0);
        FOR(i,n+1,n+n) mf.add_edge(i, n+n+2, 1, 0);
