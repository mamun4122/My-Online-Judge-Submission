...
info call(info a,info b)
{
    ...
    tmp.sum=a.sum+b.sum+mn*2;
    a.open-=mn;
    b.close-=mn;
    tmp.open=a.open+b.open;
    tmp.close=a.close+b.close;
    return tmp;
}
...
void init(int node,int b,int e)
{
 if(b==e)
 {
     tree[node].sum=tree[node].open=tree[node].close=0;
     if(arr[b])tree[node].open=1;
...
//    ios_base::sync_with_stdio(0);cin.tie(0);
    demo.sum=demo.open=demo.close=0;
    while(cin>>s)
    {
        repI(i,s.size())arr[i+1]=(s[i]=='('?1:0);
        n=s.size();
        init(1,1,n);
        getI(m);
        rep(i,m)
